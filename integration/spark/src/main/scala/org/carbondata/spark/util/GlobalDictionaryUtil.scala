...
object GlobalDictionaryUtil extends Logging {
  ...
  def getPrimDimensionWithDict(carbonLoadModel: CarbonLoadModel,
    ...
    gatherDimensionByEncoding(carbonLoadModel, dimension, Encoding.DICTIONARY, Encoding.DIRECT_DICTIONARY,
   ...
   */
  private def getPredefinedColumnDictPath(carbonLoadModel: CarbonLoadModel,
  ...
  def setPredefineDict(carbonLoadModel: CarbonLoadModel,
    ...
    val dimParent = parentDimName + {
      colName match {
        case "" => colName
        case _ =>
          if (parentDimName.isEmpty) colName.split("\\.")(0)
          else "." + colName.split("\\.")(0)
      }
    }
        ...
        preDictDimension.getDataType match {
          case DataType.ARRAY =>
            if (children(0).isComplex) "val." +
              colName.substring(colName.split("\\.")(0).length + 1)
            else "val"
          case _ => colName.substring(colName.split("\\.")(0).length + 1)
        }
      }
  ...
  private def generatePredefinedColDictionary(colDictFilePath: String,
    ...
    getPredefinedColumnDictPath(carbonLoadModel, colDictFilePath, table, dimensions)
    ...
    val extInputRDD = new CarbonColumnDictGenerateRDD(carbonLoadModel, sqlContext.sparkContext,
      table, dimensions, hdfsLocation, dictFolderPath)
