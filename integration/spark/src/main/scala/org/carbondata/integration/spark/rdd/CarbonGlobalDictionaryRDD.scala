...
class CarbonColumnDictGenerateRDD(carbonLoadModel: CarbonLoadModel,
                                  ...
                                  hdfsLocation: String,
                                  dictFolderPath: String)
  extends RDD[(Int, Array[Array[Byte]])](sparkContext, Nil) with Logging {
  ...
  : Iterator[(Int, Array[Array[Byte]])] = {
    ...
    val distinctValues = new ArrayBuffer[(Int, HashSet[Array[Byte]])]
    val dictModel = createDictionaryLoadModel(carbonLoadModel, table, Array(theSplit.colDimension),
      hdfsLocation, dictFolderPath)
    val mapIdWithSet = new HashMap[String, HashSet[Array[Byte]]]
    val columnValues = new Array[HashSet[Array[Byte]]](dictModel.primDimensions.length)
